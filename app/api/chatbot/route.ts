...
const knowledgeBase: KnowledgeCategory[] = [
  {
    triggers: [
      ...
      ['ça va', 'comment tu vas', 'comment allez-vous']
    ],
    responses: [
  ...
  {
    triggers: [
      ['c\'est quoi', 'qu\'est-ce que', 'définition', 'explique', 'définis'],
      ['ia', 'intelligence artificielle', 'l\'intelligence artificielle', 'ai']
    ],
    responses: [
  ...
  {
    triggers: [
      ['comment', 'fonctionne', 'marche', 'travailler', 'principe', 'mécanisme'],
      ['ia', 'elle', 'ça']
    ],
    responses: [
  ...
  {
    triggers: [
      ['exemple', 'application', 'utilise', 'concret', 'cas'],
      ['ia', 'dans la vie', 'quotidien', 'réel']
    ],
    responses: [
  ...
  {
    triggers: [
      ['apprendre', 'étudier', 'commencer', 'débutant', 'formation'],
      ['ia', 'machine learning', 'programmation']
    ],
    responses: [
  ...
  {
    triggers: [
      ['avantage', 'bénéfice', 'utile', 'pourquoi', 'intérêt'],
      ['ia', 'elle', 'l\'ia']
    ],
    responses: [
  ...
  {
    triggers: [
      ['danger', 'risque', 'problème', 'mauvais', 'négatif'],
      ['ia', 'elle', 'l\'ia']
    ],
    responses: [
...
// Réponses créatives
const creativeResponses: string[] = [
...
export async function POST(req: Request) {
  try {
    ...
    const userText: string = lastMessage.content.toLowerCase();
    ...
    const words: string[] = cleanText.split(' ').filter(word => word.length > 2);
