...
import ChatbotDiagram from "@/components/chatbot-diagram"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chatbot", {
        ...
        body: JSON.stringify({ messages: nextMessages }), // ← correct key
      })
      ...
      setMessages([...nextMessages, { role: "assistant", content: `Désolé, une erreur s'est produite : ${err.message}` }])
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="bg-slate-800/50 border-slate-700 p-6">
            ...
            <p className="text-slate-300 text-sm">
              Le chatbot utilise un modèle de langage avancé pour comprendre ton message et générer une réponse cohérente
              basée sur les patterns dans les données d'entraînement.
            </p>
          </Card>
          <Card className="bg-slate-800/50 border-slate-700 p-6">
